...
export class ApiaryRecord {
    ...
    static async checkIfIdExistsInDataBase(startTime: string, dailyNumber: string): Promise<Boolean> {
        const [results] = await pool.execute('SELECT `dailyNumber` FROM `apiaries` WHERE `startTime`=:startTime AND `dailyNumber`=:dailyNumber', {
            startTime,
            dailyNumber,
